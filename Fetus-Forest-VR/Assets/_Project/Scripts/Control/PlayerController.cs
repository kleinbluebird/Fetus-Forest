...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("Wwise Events")]
    public string moveUpPlay = "Move_Up_Play";
    public string moveUpStop = "Move_Up_Stop";
    public string moveDownPlay = "Move_Down_Play";
    public string moveDownStop = "Move_Down_Stop";
    ...
    private bool wasFloatingUp = false;
    private bool wasFloatingDown = false;
    ...
    {
        bool isFloatingUp = floatUpPressed;
        bool isFloatingDown = floatDownPressed;

        // --- 上升 ---
        if (isFloatingUp && !wasFloatingUp)
        {
            AkSoundEngine.PostEvent(moveUpPlay, gameObject);
        }
        else if (!isFloatingUp && wasFloatingUp)
        {
            AkSoundEngine.PostEvent(moveUpStop, gameObject);
        }

        // --- 下降 ---
        if (isFloatingDown && !wasFloatingDown)
        {
            AkSoundEngine.PostEvent(moveDownPlay, gameObject);
        }
        else if (!isFloatingDown && wasFloatingDown)
        {
            AkSoundEngine.PostEvent(moveDownStop, gameObject);
        }

        // 更新上一帧状态
        wasFloatingUp = isFloatingUp;
        wasFloatingDown = isFloatingDown;
    }
