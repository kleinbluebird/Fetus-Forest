...
namespace FetusForest.Effects
{
	...
	public class ButtonInteractionController : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerClickHandler, ISelectHandler, IDeselectHandler
	{
		...
		[Header("Wwise 音效事件")]
		[Tooltip("鼠标进入按钮时触发的 Wwise 事件名称（留空则不触发）")]
		[SerializeField] private string wwiseEventOnEnter = "";
		[Tooltip("鼠标离开按钮时触发的 Wwise 事件名称（留空则不触发）")]
		[SerializeField] private string wwiseEventOnExit = "";
		[Tooltip("鼠标点击按钮时触发的 Wwise 事件名称（留空则不触发）")]
		[SerializeField] private string wwiseEventOnClick = "";
		[Tooltip("是否在控制台显示 Wwise 事件调试信息")]
		[SerializeField] private bool showWwiseDebugLogs = false;
		[Tooltip("如果事件不存在，是否静默失败（不显示错误）。如果关闭，Wwise 会在控制台输出错误。")]
		[SerializeField] private bool silentFailOnMissingEvent = true;
		...
		public void OnPointerEnter(PointerEventData eventData)
		{
			...
			// Trigger Wwise event on mouse enter
			SafePostWwiseEvent(wwiseEventOnEnter, "OnEnter");
		}
		...
		public void OnPointerExit(PointerEventData eventData)
		{
			...
			// Trigger Wwise event on mouse exit
			SafePostWwiseEvent(wwiseEventOnExit, "OnExit");
		}
		...
		public void OnPointerClick(PointerEventData eventData)
		{
			...
			// Trigger Wwise event on mouse click
			SafePostWwiseEvent(wwiseEventOnClick, "OnClick");
		}
		...
		// ===== Wwise 事件安全触发 =====
		/// <summary>
		/// 安全地触发 Wwise 事件，如果事件不存在且启用了静默失败，则不触发
		/// </summary>
		private void SafePostWwiseEvent(string eventName, string eventType)
		{
			if (string.IsNullOrEmpty(eventName)) return;
			
			// 如果启用了静默失败，先检查事件是否存在
			if (silentFailOnMissingEvent)
			{
				uint eventId = AkSoundEngine.GetIDFromString(eventName);
				if (eventId == 0)
				{
					// 事件不存在，静默失败
					if (showWwiseDebugLogs)
					{
						Debug.LogWarning($"[ButtonInteractionController] Wwise event '{eventName}' ({eventType}) not found. Skipping to avoid errors. Make sure the event exists in your Wwise project.", this);
					}
					return;
				}
			}
			...
			// 触发事件
			uint postedEventId = AkSoundEngine.PostEvent(eventName, gameObject);
			
			if (showWwiseDebugLogs)
			{
				if (postedEventId == 0)
				{
					Debug.LogWarning($"[ButtonInteractionController] Failed to post Wwise event '{eventName}' ({eventType}). Make sure the event exists in your Wwise project and the project is properly generated.", this);
				}
				else
				{
					Debug.Log($"[ButtonInteractionController] Successfully triggered Wwise event: {eventName} ({eventType}, ID: {postedEventId})", this);
				}
			}
		}
